...
console.log('process.env.BASE_URL: ', process.env.VUE_APP_BASE_API)
...
service.interceptors.request.use(
  config => {
    ...
    if (sessionMemory.getItem('token') && config.url.indexOf(process.env.IMAGE_PREFIX) === -1) {
      // let each request carry token
      // ['X-Token'] is a custom headers key
      // please modify it according to the actual situation
      config.headers['Authorization'] = 'Bearer ' + sessionMemory.getItem('token')
    }
